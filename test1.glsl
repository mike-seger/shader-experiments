...
void mainImage( out vec4 g_FragColor, in vec2 g_FragCoord )
{
    ...
    vec2 uv = (g_FragCoord.xy-0.5*iResolution.xy) /
    ...
    {
        seg = -seg;;
        g_FragColor = vec4(seg,seg,seg,1.0);
    }
    else
        //if (showMatrix)
            if (isGreen)
                g_FragColor = vec4(0.0,seg,seg*0.5,1.0);
            else
                g_FragColor = vec4(0.0,seg*0.8,seg,1.0);
        /*else
            if (isGreen)
                g_FragColor = vec4(0.0,seg,0.0,1.0);
            else
                g_FragColor = vec4(seg,0.0,0.0,1.0);*/
...
//     mainImage( gl_g_FragColor, gl_g_FragCoord.xy );
//     //color.w = 1.0;
//     //gl_g_FragColor = color;
